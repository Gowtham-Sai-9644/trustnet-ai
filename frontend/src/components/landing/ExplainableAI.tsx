...
export const ExplainableAI: React.FC = () => {
  ...
  return (
    <section className="py-24 px-6 md:px-12 bg-transparent border-y border-landing-border">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
        <div>
          <div className="bg-fuchsia-100 text-fuchsia-600 p-3 rounded-xl inline-block mb-6">
            <FileSearch className="w-6 h-6" />
          </div>
          <h2 className="text-3xl md:text-5xl font-bold text-landing-text tracking-tight mb-6">
            We don't just guess. <br/>We explain.
          </h2>
          <p className="text-landing-muted text-lg mb-8">
        ...
        <div className="space-y-4">
          {explanations.map((exp) => (
            <div 
              key={exp.id} 
              className={`border rounded-2xl overflow-hidden bg-landing-card border border-landing-border backdrop-blur-md transition-colors cursor-pointer ${expandedId === exp.id ? 'border-purple-300 shadow-[0_8px_30px_rgb(0,0,0,0.04)]' : 'border-landing-border hover:border-slate-300'}`}
              onClick={() => setExpandedId(expandedId === exp.id ? null : exp.id)}
            >
              <div className="p-5 flex justify-between items-center">
                <span className="font-semibold text-landing-text">{exp.title}</span>
                <ChevronDown className={`w-5 h-5 text-landing-muted transition-transform ${expandedId === exp.id ? 'rotate-180' : ''}`} />
              </div>
              <AnimatePresence>
                {expandedId === exp.id && (
                  <motion.div 
                    ...
                    className="px-5 pb-5 pt-1 border-t border-landing-border text-landing-muted"
                  >
                    {exp.detail}
                  </motion.div>
                )}
