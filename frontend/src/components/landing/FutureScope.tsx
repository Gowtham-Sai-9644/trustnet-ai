...
export const FutureScope: React.FC = () => {
  const scope = [
    { title: "Browser Extension", icon: <Chrome className="w-6 h-6 text-blue-500" /> },
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
          {scope.map((s, i) => (
            <motion.div 
              ...
              className="p-6 border border-slate-200 rounded-2xl bg-slate-50 hover:bg-white hover:shadow-md transition-all group cursor-default"
            >
              <div className="bg-white p-3 rounded-xl shadow-sm inline-block mb-4 border border-slate-100 group-hover:scale-110 transition-transform">
                {s.icon}
              </div>
