...
const DashboardPage: React.FC = () => {
  ...
  const initialCases: CaseRecord[] = [
    { id: 'TXN-94721', category: 'UPI Fraud', score: 0.98, status: 'Escalated', timestamp: '09:41', assignee: 'Gowtham Sai' },
    { id: 'TXN-88413', category: 'Investment Scam', score: 0.87, status: 'Investigating', timestamp: '09:32', assignee: 'Gowtham Sai' },
    { id: 'TXN-72091', category: 'Marketplace Scam', score: 0.54, status: 'Open', timestamp: '09:15', assignee: 'Unassigned' },
    { id: 'TXN-69112', category: 'Fake Job Scam', score: 0.94, status: 'Escalated', timestamp: '08:50', assignee: 'SOC Analyst A' },
    { id: 'TXN-54210', category: 'KYC Fraud', score: 0.76, status: 'Resolved', timestamp: '08:12', assignee: 'SOC Analyst B' },
  ];
  ...
  useEffect(() => {
    ...
    // Telemetry update simulation loop (realistic background updates)
  ...
  const quickActions = [
    ...
    { title: 'Graph Workspace', path: '/console/graph', icon: Network, color: 'text-[#06B6D4]', bg: 'bg-[#06B6D4]/10' },
    ...
    <div className="space-y-4">
      ...
      <AppPageHeader 
        ...
        rightElement={
          <div className="flex items-center space-x-2 bg-[#1E293B]/40 px-3 py-1.5 rounded-xl border border-[#1E293B] text-[10px] text-slate-400">
            ...
            <span className="font-mono">Operational Console Active</span>
          </div>
        }
      />

      {/* Operations Warning Banner (Above fold summary) */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 border border-[#EF4444]/20 rounded-xl bg-[#EF4444]/5 text-xs text-[#EF4444] font-medium gap-3 text-left">
        <div className="flex items-center space-x-2">
          <AlertTriangle className="w-4 h-4 flex-shrink-0" />
          <span><strong>Threat Notice:</strong> Suspicious spikes detected in merchant-scam-address-24@ybl. 3 critical case escalations pending action.</span>
        </div>
        <button 
          onClick={() => navigate('/console/graph')}
          className="text-[10px] font-bold text-[#06B6D4] hover:underline flex-shrink-0 whitespace-nowrap cursor-pointer"
        >
          View Fraud Graph
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <AppMetricCard 
          ...
          insightText="Awaiting manager approval"
        />
        <AppMetricCard 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        {/* Left Column: Charts and Cases (Density Optimized) */}
        <div className="lg:col-span-8 space-y-4 flex flex-col">
          
          {/* Charts Row */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Detection trends */}
            <AppCard className="p-4 space-y-2 text-left">
              ...
              <div className="h-[140px] w-full text-[8px] font-mono">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={trendData} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: '#0F172A', borderColor: '#1E293B', fontSize: 8 }} />
                    <Area type="monotone" dataKey="UPI Fraud" stroke="#06B6D4" strokeWidth={1.5} fillOpacity={1} fill="url(#colorFraud)" />
            ...
            <AppCard className="p-4 space-y-2 text-left">
              <span className="font-sans font-bold text-[10px] text-slate-400 uppercase tracking-wider flex items-center space-x-1.5">
                ...
                <span>Classifier Accuracies</span>
              </span>
              <div className="h-[140px] w-full text-[8px] font-mono">
                <ResponsiveContainer width="100%" height="100%">
          ...
          {/* Active Cases Table (What requires attention) */}
          <AppCard className="p-4 space-y-3 text-left">
                ...
                },
                {
                  ...
                  render: (row) => (
                    <div className="flex items-center space-x-1.5">
                      <span className={`w-1.5 h-1.5 rounded-full ${row.score >= 0.8 ? 'bg-[#EF4444]' : row.score >= 0.5 ? 'bg-[#F59E0B]' : 'bg-[#06B6D4]'}`} />
                ...
                },
                {
                  ...
                  render: (row) => (
                    <button 
                      ...
                      className="text-[10px] text-[#06B6D4] font-semibold hover:underline cursor-pointer flex items-center"
                    >
        ...
        {/* Right Column: Live Telemetry Feed & Workspace shortcuts */}
        <div className="lg:col-span-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              {quickActions.map((action, idx) => (
                <button
                  ...
                  className="flex items-center space-x-2 bg-[#0F172A] p-2.5 rounded-xl border border-[#1E293B] hover:border-[#06B6D4]/30 transition-all text-left cursor-pointer active:scale-95"
                >
          ...
          {/* System Health */}
          <AppCard className="p-4 space-y-2.5 text-left">
            <span className="font-sans font-bold text-[10px] text-slate-400 uppercase tracking-wider block border-b border-[#1E293B]/60 pb-2">
              System Clusters
            </span>
            <div className="space-y-2 pt-1 font-mono text-[9px]">
              <div className="flex justify-between items-center bg-[#0F172A] p-2 rounded-xl border border-[#1E293B]">
          ...
          {/* Integration Status / Telemetry counts */}
          <AppCard className="p-4 space-y-3 text-left">
            <span className="font-sans font-bold text-[10px] text-slate-400 uppercase tracking-wider block border-b border-[#1E293B]/60 pb-2">
              Database Stats
            </span>
