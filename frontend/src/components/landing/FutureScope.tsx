...
export const FutureScope: React.FC = () => {
  const scope = [
    ...
    { title: "Multilingual Support", icon: <Globe className="w-6 h-6 text-landing-accent" /> },
  ...
  return (
    <section className="py-24 px-6 md:px-12 bg-landing-card border border-landing-border backdrop-blur-md">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
          {scope.map((s, i) => (
            <motion.div 
              ...
              className="p-6 border border-landing-border rounded-2xl bg-transparent hover:bg-landing-card border border-landing-border backdrop-blur-md hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all group cursor-default"
            >
              <div className="bg-landing-card border border-landing-border backdrop-blur-md p-3 rounded-xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] inline-block mb-4 border border-landing-border group-hover:scale-110 transition-transform">
                {s.icon}
              </div>
              <h3 className="font-semibold text-landing-text">{s.title}</h3>
            </motion.div>
          ))}
        </div>
      </div>
